...
{
	[TestFixture]
	public class fsharpCommentParserTests
	{
		private fsharpFileParser _parser;
		...
		public void Setup()
		{
			...
			_parser = new fsharpFileParser(_cache);
		...
		{
			Assert.That(_cache.Classes.FirstOrDefault(x => x.Name.Equals("fsharpComments")), Is.Null);
		}
		...
		{
			return File.ReadAllText(Path.Combine(Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), "TestResources"), "fsharpComments.txt"));
		}
	}
